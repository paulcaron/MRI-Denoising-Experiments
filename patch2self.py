...
Original file is located at
    https://colab.research.google.com/drive/1SXAAB5Y2uZ95WqulLgZZxnNI3C3xC3V0
"""
...
if __name__ == "__main__":
    ...
    example_filename1 = "~/MRIdenoising/Datab1000_session1.nii"
    example_filename2 = "~/MRIdenoising/Datab1000_session2.nii"
    example_filename3 = "~/MRIdenoising/Datab1000_session3.nii"

    data, _ = load_nifti(example_filename1)

    n_volumes = 20

    useful_data = np.array(data[:,:,:,:n_volumes])
    useful_data = (useful_data - useful_data.min())/(useful_data.max() - useful_data.min())

    sigma = 5./255.

    noise = np.random.normal(scale=sigma, size=useful_data.shape)

    noisy_data = useful_data + noise

    s3s_p2s = patch2self(noisy_data)

    import h5py

    h5f = h5py.File('~/data_5.h5', 'w')

    h5f.create_dataset('dataset_noisy', data=noisy_data)

    h5f.create_dataset('dataset_denoised', data=s3s_p2s)
